...
def train_and_evaluate_model(X: np.ndarray, y: np.ndarray) -> None:
    ...
    for fold, (train_index, test_index) in enumerate(kf.split(X), start=1):
        ...
        grid_search = GridSearchCV(estimator=mlp, param_grid=param_grid, scoring='auc', cv=KFOLDS, n_jobs=-1, verbose=1)
        grid_search.fit(X_train_fold, y_train_fold)

        best_mlp = grid_search.best_estimator_
        logger.info(f"Best parameters for fold {fold}: {grid_search.best_params_}")

        # Evaluate the model on the test set of this fold
        y_predictions = best_mlp.predict(X_test_fold)
        accuracy = np.mean(y_predictions == y_test_fold)
